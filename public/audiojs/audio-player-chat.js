...
function AudioPlayer(options) {
  ...
  this.bgColour = options.bgColour === undefined ? "#eee" : options.bgColour;
  this.hlColour = options.hlColour === undefined ? "#000" : options.hlColour;
  this.hvColour = options.hvColour === undefined ? "#fff" : options.hvColour;
      ...
      self._audioContext.decodeAudioData(request.response, function (buffer) {
        ...
        if (self._audioBuffer.numberOfChannels > 1) {
          ...
          for (var i = 0; i < leftChannel.length; i++) {
            self._audioData[i] = (leftChannel[i] + rightChannel[i]) / 1;
          }
        } else {
          //Mono
